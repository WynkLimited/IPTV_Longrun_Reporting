...
def get_the_latest_sizes():
    latest_sizes = '{}'
    filename = './files/buffers.txt'
    with open(filename, 'r') as file:
        lines = file.readlines()
    if(len(lines))>0:
        latest_sizes = lines[0].strip()
    return eval(latest_sizes)

global_file_sizes = get_the_latest_sizes()

def writeIntoFile(content):
    with open('./files/buffers.txt', 'w') as file:
...
def isTheFileSizeIncreasing(file_path, initial_size):
    ...
    if file_path in latest_file_sizes:
        latest_file_sizes[file_path] = current_size
    else:
        latest_file_sizes.setdefault(file_path, current_size)
...
def setInitialSize(file):
    if file in latest_file_sizes:
        global_file_sizes[file] = os.path.getsize(file)
    else:
        global_file_sizes.setdefault(file, os.path.getsize(file))
...
try:
    ...
    writeIntoFile(latest_file_sizes)